...
| 15 | rotki — `github.com/rotki/rotki` | AGPL-3.0 | 深浅 | 资产追踪中表格、状态与货币数字的清晰组织 | 采用终端绿的低饱和版本；不采用加密资产图形语言 |
...
所有主题都坚持纯色背景、纯色面板和弱阴影，不使用花哨渐变。红色固定表示上涨/买盘方向，绿色固定表示下跌/卖盘方向；主题强调色只用于选中、焦点和链接，不与红绿语义争夺注意力。
...
| 终端绿 `terminal` | 黑绿深色 | `#07100c` / `#0c1712` / `#e5f2eb` / `#9db5a7` | `#48c78e` / `#ff736a` / `#35c68c` | Redash、rotki、开发者终端 |

## 4. 可读性与实现决策

1. 三套浅色、四套深色，覆盖白、灰白、米黄、纯黑、石墨、夜蓝和黑绿；不再保留容易显得花哨的紫色主题。
2. 面板取消装饰性渐变，主要靠 `background / panel / surface / control / line` 五级语义令牌形成层次。
3. 浅色正文和次要文字均使用深色，不用低对比浅灰；深色主题次要文字提升亮度，避免长时间盯盘时“灰成一片”。
4. 主题按钮改为纯色双环样本，选中时有 2px 外轮廓，键盘焦点有 3px 强调色焦点环；按钮持续同步 `aria-pressed`。
5. 旧偏好平滑迁移：`ocean → midnight`，`violet → slate`，其他主题键保持不变。
6. 主题仍写入 `market-liquidity-radar-preferences-v2`，刷新后恢复；切换后触发现有 ECharts 重绘路径。

## 5. 未采用方向

- 不采用玻璃拟态、透明毛玻璃和背景照片：降低文字/行情数据对比度，也增加合成开销。
- 不采用大面积紫、荧光蓝或多段渐变：与红涨绿跌及资金赛马颜色竞争。
