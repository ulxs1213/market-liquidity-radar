...
- 全市场核心标的采用真实 `f5` 成交量（手）纵轴与成交量候选排序，横轴为涨跌幅、气泡面积为成交额；行业/概念图例、气泡和悬浮标签同色，涨跌和主力资金分别保持正红负绿语义。
- 个股价格、均价、分钟成交量、累计主力资金、资金分钟变化与当前五档盘口；价格/量/资金三层共用同一分钟游标并显示完整读数，可在页面底部查看，也可弹窗放大。
- 完整 A 股交易分钟轴：09:30–11:30 与 13:00–15:00；午休不补点，未来分钟留白，绝不延长到 15:30。
- 可选 09:15–09:29 集合竞价；只有东方财富本次真实返回竞价观察点时才允许开启，不插值、不模拟。
- 九套高对比预设：明纸、雾灰、暖沙、墨黑、石墨、夜蓝、酒红、浅红、绚彩；5浅4深。另有可拖拽 HSV 渐变色板，可调色相、HEX、名称和浅/深模式，显式保存后保留在本机。
- 10列紧凑模块网格支持八方向缩放，只保存列跨度和高度；模块保持12px紧贴，不保存越拉越大的空洞。
- 板块排名、赛马、核心股、全市场标的和重点队列支持双击打开完整05B；关闭按钮、Esc或点击遮罩返回原位，页面始终只保留一个图表实例。
- 3 秒轻量刷新、12 秒重模块刷新；交互和滚动时继续取数但推迟重绘，减少卡顿。
- SQLite WAL 保存本机实际观察到的分钟轨迹，默认保留 14 个自然日。页面级历史回放支持日期、播放/暂停、1×/2×/5×、真实帧进度和回到实时；01–05按同一真实分钟联动，缺口不插值、不沿用上一帧。
...
- 板块/个股横截面：`push2.eastmoney.com/api/qt/clist/get`，失败时使用 `push2delay`；板块或个股代码、涨跌、`f5` 成交量（手）、成交额、量比、换手、行业、地域板块、概念和 `f62` 主力净流等字段。全市场核心标的请求使用 `fid=f5`，候选池最多500只且不会被表述成全市场全量。
...
- 自定义取色拖动只用 `requestAnimationFrame` 更新 CSS 预览；提交换肤时先清理旧 Tooltip，再错峰重绘现有图表实例，不因换色重复初始化 ECharts。
...
主题改造的真实页面截图、保存/取消/刷新步骤与自动化结果见 [2026-07-14 主题验收](docs/audits/2026-07-14-custom-theme/AUDIT.md)。
